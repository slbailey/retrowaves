...
- Switching must complete within bounded time (≤1 frame period, ~21.3 ms)
...
- AudioPump must continue generating frames at real-time pace during switches
...
- Switching must complete within one frame period (~21.3 ms)
...
**I9: AudioPump Independence**
- AudioPump must continue running at real-time pace during source switches
- AudioPump timing must not depend on source switching operations
- Mixer must never block AudioPump frame acquisition
- AudioPump must receive frames at consistent rate (one per ~21.3 ms)
