...
class TowerConfig:
    ...
    encoder_stall_threshold_ms: int = 2000  # Stall if 0 bytes for 2 seconds (drain thread detection)
    encoder_stall_ms: int = 3000  # Stall detection threshold for monitor loop (3 seconds)
    
    # Jitter buffer configuration
    encoder_jitter_min_chunks: int = 15  # Required buffer level to start streaming
    encoder_jitter_recover_chunks: int = 10  # Required buffer level to resume after underflow
    encoder_target_chunk_rate: int = 1024  # Target bytes per read interval
    encoder_jitter_read_interval_ms: float = 15.0  # Fixed read interval (15ms) for steady output
    ...
    def load_config(cls) -> "TowerConfig":
        ...
        encoder_stall_threshold_ms_str = os.getenv("TOWER_ENCODER_STALL_THRESHOLD_MS", "2000")
        try:
            encoder_stall_threshold_ms = int(encoder_stall_threshold_ms_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_STALL_THRESHOLD_MS: {encoder_stall_threshold_ms_str} (must be an integer)")
        
        encoder_stall_ms_str = os.getenv("TOWER_ENCODER_STALL_MS", "3000")
        try:
            encoder_stall_ms = int(encoder_stall_ms_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_STALL_MS: {encoder_stall_ms_str} (must be an integer)")
        
        # Jitter buffer configuration
        encoder_jitter_min_chunks_str = os.getenv("TOWER_ENCODER_JITTER_MIN_CHUNKS", "15")
        try:
            encoder_jitter_min_chunks = int(encoder_jitter_min_chunks_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_JITTER_MIN_CHUNKS: {encoder_jitter_min_chunks_str} (must be an integer)")
        
        encoder_jitter_recover_chunks_str = os.getenv("TOWER_ENCODER_JITTER_RECOVER_CHUNKS", "10")
        try:
            encoder_jitter_recover_chunks = int(encoder_jitter_recover_chunks_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_JITTER_RECOVER_CHUNKS: {encoder_jitter_recover_chunks_str} (must be an integer)")
        
        encoder_target_chunk_rate_str = os.getenv("TOWER_ENCODER_TARGET_CHUNK_RATE", "1024")
        try:
            encoder_target_chunk_rate = int(encoder_target_chunk_rate_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_TARGET_CHUNK_RATE: {encoder_target_chunk_rate_str} (must be an integer)")
        
        encoder_jitter_read_interval_ms_str = os.getenv("TOWER_ENCODER_JITTER_READ_INTERVAL_MS", "15.0")
        try:
            encoder_jitter_read_interval_ms = float(encoder_jitter_read_interval_ms_str)
        except ValueError:
            raise ValueError(f"Invalid TOWER_ENCODER_JITTER_READ_INTERVAL_MS: {encoder_jitter_read_interval_ms_str} (must be a float)")
        ...
        config = cls(
            ...
            encoder_stall_threshold_ms=encoder_stall_threshold_ms,
            encoder_stall_ms=encoder_stall_ms,
            encoder_jitter_min_chunks=encoder_jitter_min_chunks,
            encoder_jitter_recover_chunks=encoder_jitter_recover_chunks,
            encoder_target_chunk_rate=encoder_target_chunk_rate,
            encoder_jitter_read_interval_ms=encoder_jitter_read_interval_ms,
    ...
    def validate(self) -> None:
        ...
        # Validate encoder stall threshold
        if self.encoder_stall_threshold_ms <= 0:
            raise ValueError(f"Invalid encoder stall threshold: {self.encoder_stall_threshold_ms} (must be > 0)")
        
        # Validate encoder stall ms
        if self.encoder_stall_ms <= 0:
            raise ValueError(f"Invalid encoder stall ms: {self.encoder_stall_ms} (must be > 0)")
        
        # Validate jitter buffer configuration
        if self.encoder_jitter_min_chunks <= 0:
            raise ValueError(f"Invalid encoder_jitter_min_chunks: {self.encoder_jitter_min_chunks} (must be > 0)")
        if self.encoder_jitter_recover_chunks <= 0:
            raise ValueError(f"Invalid encoder_jitter_recover_chunks: {self.encoder_jitter_recover_chunks} (must be > 0)")
        if self.encoder_jitter_recover_chunks >= self.encoder_jitter_min_chunks:
            raise ValueError(f"encoder_jitter_recover_chunks ({self.encoder_jitter_recover_chunks}) must be < encoder_jitter_min_chunks ({self.encoder_jitter_min_chunks})")
        if self.encoder_target_chunk_rate <= 0:
            raise ValueError(f"Invalid encoder_target_chunk_rate: {self.encoder_target_chunk_rate} (must be > 0)")
        if self.encoder_jitter_read_interval_ms <= 0:
            raise ValueError(f"Invalid encoder_jitter_read_interval_ms: {self.encoder_jitter_read_interval_ms} (must be > 0)")
