...
- Tone generator must produce frames at real-time pace (exactly 21.333 ms intervals)
...
- **Note:** In Phase 1, tone generator timing may be implemented directly in the PCM writer thread. In Phase 3+, timing is owned exclusively by AudioPump, not the generator.
...
- **Note:** Synchronous in Phase 1 only — may block on slow clients. Phase 2+ will introduce non-blocking / drop-slow-clients behavior.
...
**7.3 PCM Writer Thread (AudioPump)**
- Tower must run a dedicated thread for writing PCM frames to FFmpeg stdin
- PCM writer thread (AudioPump) must generate tone frames at exactly 21.333ms intervals
- PCM writer thread must use absolute clock timing to prevent cumulative drift
- PCM writer thread must write frames to FFmpeg stdin continuously
- PCM writer thread must handle pipe errors gracefully
- **Note:** In Phase 3+, this becomes the AudioPump component which is Tower's sole metronome
...
- Tower should prefer staying alive and streaming tone rather than exiting on non-critical errors
