...
**Key Architecture Principle:** Tower is the sole metronome in the system. Station writes frames with no timing (unpaced bursts), while Tower consumes frames at exactly 21.333ms intervals using absolute clock timing. Tower's steady consumption rate stabilizes the bounded buffer and prevents unbounded growth.
...
- ✅ Bounded queue (size 10, ~213ms) with overflow handling (drop newest frame)
- ✅ Frame size validation (4096-byte boundaries) and safe discard of malformed frames
- ✅ Timeout-based fallback with grace period (5 seconds) when no writer connected or frames unavailable
...
- **Router reads continuously without blocking:** The read thread loops continuously regardless of queue state. Decode speed never pauses. Router reads from writer socket immediately and continuously, processing frames as fast as Station can send them. This ensures Station's unpaced burst writes are never throttled by Tower's read loop.

**2.4 Bounded Queue (Ring Buffer for Jitter Handling)**
- AudioInputRouter must maintain a bounded queue (ring buffer)
- Queue size: 10 frames (~213 ms of audio at 48 kHz)
...
- Queue must be bounded (maximum 10 frames)
- Tower's steady consumption rate (21.333ms per frame) stabilizes the buffer
- **Ring buffer absorbs jitter/burstiness from Station's unpaced writes** (ARCHITECTURE_TOWER.md Section 4.1)
- Buffer must handle variable input timing (jitter) while Tower pulls at steady 21.333ms intervals
- Buffer never grows unbounded — Tower consumption rate stabilizes the system
- Buffer must maintain continuous output despite variable input timing (jitter absorption)

**2.5 Queue Overflow Handling**
- When queue is full (10 frames) and a new frame arrives:
  ...
  - AudioInputRouter must maintain queue size at exactly 10 frames
- **Why drop newest, not oldest?** Tower is a live transmitter, not a history buffer. We must always play the most recent audio available. Dropping newest frame preserves real-time feel and keeps Tower synced to current audio, ensuring listeners hear the latest content without delay.
- Queue overflow must not propagate backpressure to writer:
  - Writer must be allowed to write without blocking (Station writes unpaced bursts)
...
- Station writes frames with no timing (unpaced, immediate writes as decoded)
- Tower's steady pull rate (21.333ms) absorbs burstiness and prevents sustained overflow
...
**3.2 Frame Acquisition Logic (with Grace Period)**
- AudioPump must attempt `AudioInputRouter.get_next_frame(timeout_ms)` first
  - Timeout must be short (~5ms) to minimize blocking
  - AudioPump must use `get_nowait()` if queue has frames (non-blocking)
  - Only use timeout if queue is empty
- AudioPump must check grace period before falling back:
  - If `get_next_frame()` returns a frame: use live PCM frame
  - If `get_next_frame()` returns `None`:
    - Check if within grace period (default 5 seconds): if yes, use silence frame
    - If grace period expired: fall back to ToneSource (or current SourceManager source)
- AudioPump must write the chosen frame to FFmpeg stdin immediately
- AudioPump must never stall, block longer than timeout, or shift its pacing
- AudioPump timing is independent of router state — Tower is the sole metronome

**3.3 Real-Time Pacing (Tower is the Sole Metronome)**
- AudioPump must generate/forward exactly 1 frame every 21.333 ms (exact, not approximate)
- Frame period: `1024 samples / 48000 Hz = 21.333 ms` (exact)
- AudioPump must use absolute clock timing to prevent cumulative drift:
  - Initialize `next_frame_time = time.time()` before loop
  - Each iteration: `next_frame_time += FRAME_DURATION`
  - Sleep for `next_frame_time - time.time()` (remaining time in period)
  - If behind schedule, resync: `next_frame_time = time.time()`
- AudioPump must maintain real-time pace regardless of Station write pattern
- AudioPump timing must be independent of router, network, writer, or filesystem
...
**4.4 No Frame Received Before Timeout (with Grace Period)**
- When `AudioInputRouter.get_next_frame(timeout_ms)` times out:
  - AudioInputRouter must return `None`
  - AudioPump must check grace period:
    - If within grace period (default 5 seconds): use silence frame
    - If grace period expired: use fallback source (tone/file)
  - Tower must continue streaming without gaps
- Timeout must be short (~5ms) to minimize blocking (Tower is the metronome)
- **Grace period prevents tone blips during short gaps between MP3 tracks**
  - When Station switches between MP3 files, there may be brief gaps in PCM data
  - Tower must wait 5 seconds (configurable via `TOWER_PCM_GRACE_SEC`) before switching to fallback tone
  - During the grace period, Tower uses silence frames to maintain continuous MP3 stream
  - After grace period expires, Tower switches to fallback tone source
  - This ensures smooth transitions between tracks without audible tone interruptions
- Grace period is configurable via `TOWER_PCM_GRACE_SEC` (default: 5 seconds)
...
**I4: Queue Bounds**
- Queue must never exceed 10 frames
- Queue must drop newest frame on overflow (not oldest)
- Queue must maintain size ≤ 10 at all times
- Tower's steady consumption rate (21.333ms per frame) stabilizes the buffer
- Buffer absorbs burstiness from Station's unpaced writes
...
**I13: Real-Time Pacing (Tower is the Sole Metronome)**
- AudioPump must generate/forward exactly 1 frame every 21.333 ms (exact, not approximate)
- AudioPump must use absolute clock timing to prevent cumulative drift
- AudioPump must maintain real-time pace regardless of Station write pattern
- AudioPump timing must be independent of router, network, writer, or filesystem
...
**I15: AudioPump Pacing Independence (Tower is the Sole Metronome)**
- AudioPump runs strictly at 1024/48000 Hz period (exactly 21.333 ms per frame)
- AudioPump must use absolute clock timing (`next_frame_time += FRAME_DURATION`) to prevent drift
- Timing must not depend on router, network, writer, or filesystem
- Router latency must NEVER influence AudioPump's frame cadence
- AudioPump must NOT wait for writer — it must always:
  - Try `router.get_next_frame(timeout)` with short timeout (~5ms)
  - Use `get_nowait()` if queue has frames (non-blocking)
  - If timeout/None → check grace period → silence or fallback immediately
- AudioPump must NEVER:
  - Stall
  - Block longer than timeout
  - Shift its pacing
  - Accumulate latency (absolute clock timing prevents drift)
- This keeps Tower rock-solid and prevents FFmpeg from falling behind or crashing
- Station writes unpaced bursts; Tower pulls steady (metronome)
...
- **Section 2 (AudioInputRouter)** → Queue behavior tests, writer connection tests, frame integrity tests, thread-safety tests, ring buffer jitter-handling tests
...
- ❌ Queue size configuration (fixed at 10 frames)
...
4. ✅ AudioInputRouter maintains bounded queue of size 10 (~213ms)
...
8. ✅ AudioPump attempts `AudioInputRouter.get_next_frame(timeout_ms)` with short timeout (~5ms), uses `get_nowait()` if queue has frames
9. ✅ AudioPump uses grace period (5 seconds, configurable via `TOWER_PCM_GRACE_SEC`) before falling back to ToneSource
10. ✅ AudioPump generates/forwards 1 frame every exactly 21.333 ms (Tower is the sole metronome, uses absolute clock timing to prevent drift)
