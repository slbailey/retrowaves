...
- EncoderManager must ensure AudioPump can continue to run at exactly 21.333ms intervals (Tower is the sole metronome)
...
- When encoder is down (RESTARTING or FAILED):
  ...
  - AudioPump must continue generating frames at exactly 21.333ms intervals (absolute clock timing)
...
- When EncoderManager enters FAILED state:
  ...
  - In FAILED state, AudioPump must continue generating silence PCM frames at the same real-time cadence (21.333ms intervals)
  - Silence MUST be encoded to MP3 in real time (not pre-buffered)
...
- **MP3 Generation Requirement**: Tower MUST NOT generate MP3 without FFmpeg. In FAILED state, Tower continues PCM generation but MP3 output depends on encoder availability. If encoder is permanently FAILED, Tower continues serving existing TCP connections but clients may hear silence only from the moment encoder is restarted manually. The stream must remain open. Tower MUST NOT terminate `/stream` connections even if audio is silent.
...
- When a slow client is dropped, broadcast loop MUST NOT skip or retry sending that chunk to others. Drop is immediate, then broadcast continues.
...
- AudioPump must continue running at exactly 21.333ms intervals during encoder restarts (Tower is the sole metronome)
- AudioPump must continue running at exactly 21.333ms intervals when clients are slow (absolute clock timing)
...
**4.5 Component Isolation Architecture**
- Tower components must maintain strict isolation to prevent backpressure propagation:
  ```
                         +---------------------------+
                         |       EncoderManager      |
  PCM (21.33ms steady) → | stdin    FFmpeg    stdout | → MP3 Chunks
                         +-------------+-------------+
                                       |
                                       v
                           +----------------------+
...
- **Critical Isolation Rules:**
  - No component above the HTTP layer may backpressure EncoderManager
  - EncoderManager must not backpressure AudioPump
  - AudioPump is the one and only metronome
  - HTTPConnectionManager must not backpressure EncoderManager reader loop
  - Slow clients must be dropped, not propagated as backpressure
...
**I12: AudioPump Independence (Tower is the Sole Metronome)**
- AudioPump continues to run at exactly 21.333ms intervals during encoder restarts (absolute clock timing)
- AudioPump continues to run at exactly 21.333ms intervals when clients are slow (absolute clock timing)
- AudioPump timing is independent of encoder state, client state, router state, and Station write pattern
...
14. ✅ AudioPump continues at exactly 21.333ms intervals during encoder restarts (Tower is the sole metronome)
