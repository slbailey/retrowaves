...
class TestFailedState:
    ...
    def test_f1_stream_serves_silence_in_failed_state(self, tower_instance):
        ...
        Assert:
        - /stream continues to serve MP3 data (silent frames).
        ...
        # Connect to /stream and verify it serves data (silent MP3)
        try:
            stream_data = collect_mp3_chunks(host, port, duration_seconds=0.5)
            assert len(stream_data) > 0, "/stream did not serve data in FAILED state"
            # Note: We can't easily verify it's silence without decoding,
            # but we can verify it's MP3 format
            assert is_valid_mp3_header(stream_data), "/stream served invalid MP3 in FAILED state"
        except Exception as e:
            pytest.fail(f"/stream failed in FAILED state: {e}")
        
        # Verify /stream remains connected (doesn't disconnect)
        try:
            with httpx.Client(timeout=2.0) as client:
                with client.stream("GET", f"http://{host}:{port}/stream") as resp:
                    assert resp.status_code == 200, "/stream returned error in FAILED state"
                    # Read a few chunks to verify connection is stable
                    chunks_read = 0
                    for chunk in resp.iter_bytes(chunk_size=8192):
                        chunks_read += 1
                        if chunks_read >= 3:
                            break
                    assert chunks_read >= 3, "Could not read multiple chunks from /stream"
        except Exception as e:
